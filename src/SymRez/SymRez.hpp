...
public:
  ...
  public:
    ...
    inline iterator(sr_iterator_t iter) : iter_(iter), current_(iter) {}
  ...
  inline explicit operator bool() const { return bool(symrez_); }

  static inline void *ResolveOnce(const std::string_view &image_name,
                                  const std::string_view &symbol) {
    return symrez_resolve_once(image_name.data(), symbol.data());
  }

  static inline void *ResolveOnce(mach_header_t &image,
                                  const std::string_view &symbol) {
    return symrez_resolve_once_mh(image, symbol.data());
  }

  template <typename ReturnType, typename... Args,
            typename FunctionType = ReturnType (*)(Args...)>
  inline FunctionType resolveSymbol(const std::string_view &symbol) const {
    return reinterpret_cast<FunctionType>(
        sr_resolve_symbol(symrez_.get(), symbol.data()));
  }

  template <typename ReturnType, typename... Args,
            typename FunctionType = ReturnType (*)(Args...)>
  inline FunctionType
  resolveExportedSymbol(const std::string_view &symbol) const {
    return reinterpret_cast<FunctionType>(
        sr_resolve_exported(symrez_.get(), symbol.data()));
  }
  ...
  inline iterator end() const { return iterator((sr_iterator_t) nullptr); }
