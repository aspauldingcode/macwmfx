...
- (instancetype)init {
    self = [super init];
    if (self) {
        MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Titlebar controller initialized");
    }
    return self;
}
...
+ (void)initialize {
    if (self == [BS_NSWindow_TitleBar class]) {
        ...
        MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Titlebar controller class initialized");
    }
}
...
+ (void)updateAllWindowTitlebars {
    MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Updating all window titlebars. Enabled=%d", gTitlebarConfig.enabled);
