    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (NSWindow *window in [NSApp windows]) {
            @try {
                ...
                MACWMFX_LOG_ERROR(macwmfx_log_titlebar, "Error updating window title: %{public}@", exception);
            }
        }
    });
}
...
- (void)makeKeyAndOrderFront:(id)sender {
    @try {
        ...
        MACWMFX_LOG_ERROR(macwmfx_log_titlebar, "Error in makeKeyAndOrderFront: %{public}@", exception);
    }
}
...
- (void)setTitle:(NSString *)title {
    @try {
        ...
        if (gCustomTitleConfig.enabled && gCustomTitleConfig.title) {
            ...
            MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Applying custom title: %{public}@", finalTitle);
        }
        ...
        MACWMFX_LOG_ERROR(macwmfx_log_titlebar, "Error setting title: %{public}@", exception);
...
- (void)setTitleWithRepresentedFilename:(NSString *)filename {
    @try {
        ...
        MACWMFX_LOG_ERROR(macwmfx_log_titlebar, "Error setting title with filename: %{public}@", exception);
