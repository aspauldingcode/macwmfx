...
- (instancetype)init {
    self = [super init];
    if (self) {
        MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Titlebar aesthetics controller initialized");
    }
    return self;
}
...
+ (void)initialize {
    if (self == [BS_NSWindow_TitleColor class]) {
        ...
        MACWMFX_LOG_INFO(macwmfx_log_titlebar, "Titlebar aesthetics controller class initialized");
    }
}
