    ...
    for (unsigned int i = 0; i < methodCount; i++) {
        ...
        if (class_respondsToSelector(destination, selector)) {
            ...
            if (strcmp(originalType, newType) != 0) {
                MACWMFX_LOG_ERROR(macwmfx_log_general, "ZKSwizzle: incompatible type encoding for %@. (expected %s, got %s)", methodName, originalType, newType);
                return NO;
            }
            ...
            if (!class_addMethod(destination, destSel, method_getImplementation(method), method_getTypeEncoding(method))) {
                MACWMFX_LOG_ERROR(macwmfx_log_general, "ZKSwizzle: failed to add method %@ onto class %@ with selector %@", NSStringFromSelector(selector), NSStringFromClass(source), NSStringFromSelector(destSel));
                return NO;
            }
